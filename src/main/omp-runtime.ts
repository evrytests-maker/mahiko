...
export async function findOmpExecutable(override?: string | null, env: NodeJS.ProcessEnv = process.env, bundledExecutable?: string | null): Promise<string | null> {
  if (override) return (await isExecutable(override)) ? override : null;
  if (bundledExecutable && await isExecutable(bundledExecutable)) return bundledExecutable;
  ...
  const candidates = [
    ...
    ...(process.platform === "win32" && env.LOCALAPPDATA ? [join(env.LOCALAPPDATA, "omp", "omp.exe")] : []),
  ];
...
export async function discoverRuntime(
  ...
  const executable = await findOmpExecutable(override, env, options.bundledExecutable);
  const base = {
