...
function fakeOmp(body: string): string {
  const directory = mkdtempSync(join(tmpdir(), "mohiko-omp-"));
...
describe("OMP version discovery", () => {
  ...
  it("reports a missing executable", async () => {
    const snapshot = await discoverRuntime(process.cwd(), lock, "/missing/mohiko-omp");
  ...
  it("does not start RPC for an incompatible version", async () => {
    const marker = join(tmpdir(), `mohiko-rpc-marker-${process.pid}-${Date.now()}`);
